...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    for (NSString *name in _contactsSource) {
        if (name) {
            [array addObject:name];
        }
    }
    [[UserProfileManager sharedInstance] loadUserProfileInBackgroundWithBuddy:array saveToLoacal:YES completion:^(BOOL success, NSError *error) {
        [self hideHud];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.mulChoice) {
        ...
        {
            if (self.maxNumber >0) {
                if (self.selectedContacts.count<self.maxNumber) {
                ...
                }else
                {
                    ...
                    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:nil message:[NSString stringWithFormat:@"最多可选%ld人",(long)self.maxNumber] delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
                    [alert show];
                }
            }else
            {
