...
@interface SettingsViewController () <JDRedPacketsDelegate>

@property (strong, nonatomic) UIView *footerView;
@property (assign, nonatomic) SelectedType type;
@property (strong, nonatomic) EMGroup *selectedGroup;
@property (strong, nonatomic) EaseUserModel *selectedModel;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        ...
        cell.textLabel.textColor = RGBACOLOR(51, 51, 51, 1.0);
        cell.textLabel.font = [UIFont systemFontOfSize:16.0];

    return cell;
}
    
#pragma mark - JDRedPacketsSDK delegate

- (void)sendRedPacketWithCurrentVC:(UIViewController *)currentVC completionBlock:(void (^)(BOOL))block
{
    JDRPContactViewController *contactViewController = [[JDRPContactViewController alloc] init];
    contactViewController.selectedBlock = ^(id something, SelectedType type) {
        self.type = type;
        if (type == SelectedTypeUser) {
            self.selectedModel = (EaseUserModel *)something;
        } else {
            self.selectedGroup = (EMGroup *)something;
        }
        if (block != nil) {
            block(YES);
        }
    ...
    [currentVC.navigationController pushViewController:contactViewController animated:YES];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        ...
        [[JDRedPacketsSDK sharedInstance] showHomePageWithSendBlock:^(NSDictionary *resultDic) {
            ...
            EMMessage *message = [EaseSDKHelper sendTextMessage:messageStr
                                                             to:self.type == SelectedTypeUser ? self.selectedModel.buddy : self.selectedGroup.groupId
                                                    messageType:self.type == SelectedTypeUser ? EMChatTypeChat : EMChatTypeGroupChat
...
- (UIView *)footerView
{
    if (_footerView == nil) {
        ...
        UIButton *logoutButton = [[UIButton alloc] initWithFrame:CGRectMake(25, 20, _footerView.frame.size.width - 50, 45)];
